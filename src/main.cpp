...
static const int MESSAGE_SIZE = 400;
...
static uint8_t telemetry_payload[MESSAGE_SIZE];
...
void increaseIndex(){
  if(parking_slot_number < 3)
    parking_slot_number = parking_slot_number + 1;
  else 
...
void onHandleMessage(String received_string){
  Serial.print("[+] Data String C2D: ");
  Serial.printf(received_string.c_str());
  deserializeJson(doc, received_string);
  const uint8_t result = doc["Result"];
  const uint8_t id = doc["Id"];
  String jsonData = "[+] Data receive: " + String(result) + "," + String(id) + "\n";
...
void receivedCallback(char* topic, byte* payload, unsigned int length)
{
  ...
  {
    received_string += String((char)payload[i]);
  }
  onHandleMessage(received_string);
}
...
static char* getTelemetryPayload(int cond)
{
  switch (cond)
  {
    case RFID_IN_NUMBER:
    ...
    case NOTIFY_PARKING_NUMBER:
    {
      doc.clear();
      doc["Method"] = METHOD_NOTIFY_PARKING;
      switch (parking_slot_number)
      {
        case 0:
        {
          doc["ParkingSlotId"] = PARKING_SLOT_1;
          break;
        }
        case 1:
        {
          doc["ParkingSlotId"] = PARKING_SLOT_2;
          break;
        }
        case 2:
        {
          doc["ParkingSlotId"] = PARKING_SLOT_3;
          break;
        }
        case 3:
        {
          doc["ParkingSlotId"] = PARKING_SLOT_4;
          break;
        }
        default:
          break;
    ...
    case NOTIFY_UNPARKING_NUMBER:
    {
      doc.clear();
      doc["Method"] = METHOD_NOTIFY_UNPARKING;
      switch (parking_slot_number)
      {
        case 0:
        {
    ...
    default:
    {
      memset(telemetry_payload,0,MESSAGE_SIZE);
      break;
    }
  }
  Serial.print("[+] Message type:");
  Serial.println(mess_type);
  Serial.print("[+] Parking Slot Number:");
  Serial.println(parking_slot_number);
...
static void sendTelemetry()
{
  ...
  mqtt_client.publish(telemetry_topic, getTelemetryPayload(mess_type), false);
  increaseIndex();
  Serial.println("OK");
  delay(100);
  digitalWrite(LED_PIN, LOW);
